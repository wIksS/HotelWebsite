...
(function ($) {
	...
	jQuery.extend(true, _R, {
		...
		hideUnHideNav: function hideUnHideNav(opt) {
			var w = opt.c.width(),
		...
		resizeThumbsTabs: function resizeThumbsTabs(opt, force) {
		...
		updateNavIndexes: function updateNavIndexes(opt) {
			var _ = opt.c;
		...
		manageNavigation: function manageNavigation(opt) {
			...
			if (ckNO(opt.navigation.thumbnails)) setNavElPositions(opt.c.parent().find('.tp-thumbs'), opt.navigation.thumbnails, opt);

			if (ckNO(opt.navigation.tabs)) setNavElPositions(opt.c.parent().find('.tp-tabs'), opt.navigation.tabs, opt);
		...
		createNavigation: function createNavigation(container, opt) {
			if (_R.compare_version(extension).check === "stop") return false;
			var cp = container.parent(),
			    _a = opt.navigation.arrows,
			    _b = opt.navigation.bullets,
			    _c = opt.navigation.thumbnails,
			    _d = opt.navigation.tabs,
			    a = ckNO(_a),
			    b = ckNO(_b),
			    c = ckNO(_c),
			    d = ckNO(_d);
			...
			opt.li.each(function (index) {
				...
				if (b) if (opt.navigation.bullets.rtl) addBullet(container, _b, li_rtl, opt);else addBullet(container, _b, li, opt);

				if (c) if (opt.navigation.thumbnails.rtl) addThumb(container, _c, li_rtl, 'tp-thumb', opt);else addThumb(container, _c, li, 'tp-thumb', opt);
				if (d) if (opt.navigation.tabs.rtl) addThumb(container, _d, li_rtl, 'tp-tab', opt);else addThumb(container, _d, li, 'tp-tab', opt);
			});
			...
			container.bind('revolution.slide.onafterswap revolution.nextslide.waiting', function () {
				...
				var ai = 0,
				    f = false;
				if (opt.thumbs) jQuery.each(opt.thumbs, function (i, obj) {
					ai = f === false ? i : ai;
	...
	var moveThumbsInPosition = function moveThumbsInPosition(container, opt) {
		...
		if (!container.hasClass("dragged")) {
			if (thumbdir === "vertical") el.data('tmmove', punchgs.TweenLite.to(el, 0.5, { top: curpos + "px", ease: punchgs.Power3.easeInOut }));else el.data('tmmove', punchgs.TweenLite.to(el, 0.5, { left: curpos + "px", ease: punchgs.Power3.easeInOut }));
			el.data('offset', curpos);
		}
	};

	// RESIZE THE THUMBS BASED ON ORIGINAL SIZE AND CURRENT SIZE OF WINDOW
	var rtt = function rtt(f, tws, c, o, lis, wh) {
		...
		if (fin) jQuery.each(fin, function (i, el) {
			if (o.direction === "vertical") tws.add(punchgs.TweenLite.set(el, { top: i * (ch + parseInt(o.space === undefined ? 0 : o.space, 0)), width: cw + "px", height: ch + "px" }));else if (o.direction === "horizontal") tws.add(punchgs.TweenLite.set(el, { left: i * (cw + parseInt(o.space === undefined ? 0 : o.space, 0)), width: cw + "px", height: ch + "px" }));
		});
		return tws;
	};

	// INTERNAL FUNCTIONS
	var normalizeWheel = function normalizeWheel(event) /*object*/{
	...
	var initKeyboard = function initKeyboard(container, opt) {
	...
	var initMouseScroll = function initMouseScroll(container, opt) {
		...
		container.on('mousewheel DOMMouseScroll', function (e) {
			...
			if (opt.navigation.mouseScrollNavigation != "carousel") {
				if (opt.navigation.mouseScrollReverse !== "reverse") if (tc > 0 && res.pixelY > 0 || bc < jQuery(window).height() && res.pixelY < 0) ret = true;
				if (opt.navigation.mouseScrollReverse === "reverse") if (tc < 0 && res.pixelY < 0 || bc > jQuery(window).height() && res.pixelY > 0) ret = true;
			} else {
				ret = false;
			}
	...
	var swipeAction = function swipeAction(container, opt, vertical) {
		...
		var notonbody = _ISM || _R.get_browser() === "Firefox",
		    SwipeOn = container,
		    //notonbody ? container : jQuery('body'),
		...
		if (!jQuery.fn.swipetp.defaults || !jQuery.fn.swipetp.defaults.excludedElements) if (!jQuery.fn.swipetp.defaults) jQuery.fn.swipetp.defaults = new Object();
		...
		SwipeOn.swipetp({
			...
			swipeStatus: function swipeStatus(event, phase, direction, distance, duration, fingerCount, fingerData) {
				...
				if (opt.sliderType === "carousel" && ((phase === "move" || phase === "end" || phase == "cancel") && opt.dragStartedOverSlider && !opt.dragStartedOverThumbs && !opt.dragStartedOverTabs || phase === "start" && withinslider > 0 && withinthumbs === 0 && withintabs === 0)) {
					...
					switch (phase) {
						...
						case "move":
							...
							if (_.infinity === "off") {
								...
								if ((_.overpull === "none" || _.overpull === 0) && (bb < 0 || bb > opt.slideamount - 1)) _.overpull = distance;else if (bb >= 0 && bb <= opt.slideamount - 1 && (bb >= 0 && distance > _.overpull || bb <= opt.slideamount - 1 && distance < _.overpull)) _.overpull = 0;

								_.slide_offset = bb < 0 ? _.slide_offset + (_.overpull - distance) / 1.1 + Math.sqrt(Math.abs((_.overpull - distance) / 1.1)) : bb > opt.slideamount - 1 ? _.slide_offset + (_.overpull - distance) / 1.1 - Math.sqrt(Math.abs((_.overpull - distance) / 1.1)) : _.slide_offset;
							}
					...
					if ((phase === "move" || phase === "end" || phase == "cancel") && !opt.dragStartedOverSlider && (opt.dragStartedOverThumbs || opt.dragStartedOverTabs) || phase === "start" && withinslider > 0 && (withinthumbs > 0 || withintabs > 0)) {
						...
						switch (phase) {
							...
							case "move":
								...
								if (tdir === "vertical") punchgs.TweenLite.set(el, { top: curpos + "px" });else punchgs.TweenLite.set(el, { left: curpos + "px" });
							...
							case "cancel":
								...
								if (istt) {
									...
									if (tdir === "vertical") punchgs.TweenLite.to(el, 0.5, { top: curpos + "px", ease: punchgs.Power3.easeOut });else punchgs.TweenLite.to(el, 0.5, { left: curpos + "px", ease: punchgs.Power3.easeOut });
	...
	var hdResets = function hdResets(o) {
	...
	var ckNO = function ckNO(opt) {
		return opt && opt.enable;
	};

	var ckNOLO = function ckNOLO(opt) {
		return opt && opt.enable && opt.hide_onleave === true && (opt.position === undefined ? true : !opt.position.match(/outer/g));
	};

	var callAllDelayedCalls = function callAllDelayedCalls(container, opt) {
		var cp = container.parent();

		if (ckNOLO(opt.navigation.arrows)) punchgs.TweenLite.delayedCall(_ISM ? opt.navigation.arrows.hide_delay_mobile : opt.navigation.arrows.hide_delay, showHideNavElements, [cp.find('.tparrows'), opt.navigation.arrows, "hide"]);

		if (ckNOLO(opt.navigation.bullets)) punchgs.TweenLite.delayedCall(_ISM ? opt.navigation.bullets.hide_delay_mobile : opt.navigation.bullets.hide_delay, showHideNavElements, [cp.find('.tp-bullets'), opt.navigation.bullets, "hide"]);

		if (ckNOLO(opt.navigation.thumbnails)) punchgs.TweenLite.delayedCall(_ISM ? opt.navigation.thumbnails.hide_delay_mobile : opt.navigation.thumbnails.hide_delay, showHideNavElements, [cp.find('.tp-thumbs'), opt.navigation.thumbnails, "hide"]);

		if (ckNOLO(opt.navigation.tabs)) punchgs.TweenLite.delayedCall(_ISM ? opt.navigation.tabs.hide_delay_mobile : opt.navigation.tabs.hide_delay, showHideNavElements, [cp.find('.tp-tabs'), opt.navigation.tabs, "hide"]);
	};

	var showHideNavElements = function showHideNavElements(container, opt, dir, speed) {
	...
	// ADD ARROWS
	var initArrows = function initArrows(container, o, opt) {
		...
		if (container.find('.tp-leftarrow.tparrows').length === 0) container.append('<div class="tp-leftarrow tparrows ' + o.style + ' ' + o.left.style + '">' + o.tmp + '</div>');
		if (container.find('.tp-rightarrow.tparrows').length === 0) container.append('<div class="tp-rightarrow tparrows ' + o.style + ' ' + o.right.style + '">' + o.tmp + '</div>');
		...
		if (o.rtl) {
			...
			la.click(function () {
				opt.sc_indicator = "arrow";opt.sc_indicator_dir = 0;container.revnext();
			});
			ra.click(function () {
				opt.sc_indicator = "arrow";opt.sc_indicator_dir = 1;container.revprev();
			});
		} else {
			// CLICK HANDLINGS ON LEFT AND RIGHT ARROWS
			ra.click(function () {
		...
		o.padding_top = parseInt(opt.carousel.padding_top || 0, 0), o.padding_bottom = parseInt(opt.carousel.padding_bottom || 0, 0);
	...
	var putVinPosition = function putVinPosition(el, o, opt) {
	...
	var putHinPosition = function putHinPosition(el, o, opt) {
	...
	var setNavElPositions = function setNavElPositions(el, o, opt) {

		var wrapper = el.closest('.tp-simpleresponsive').length > 0 ? el.closest('.tp-simpleresponsive') : el.closest('.tp-revslider-mainul').length > 0 ? el.closest('.tp-revslider-mainul') : el.closest('.rev_slider_wrapper').length > 0 ? el.closest('.rev_slider_wrapper') : el.parent().find('.tp-revslider-mainul'),
		    ww = wrapper.width(),
		    wh = wrapper.height();
		...
		if (o.position === "outer-left" && (o.sliderLayout == "fullwidth" || o.sliderLayout == "fullscreen")) punchgs.TweenLite.set(el, { left: 0 - el.outerWidth() + "px", x: o.h_offset + "px" });else if (o.position === "outer-right" && (o.sliderLayout == "fullwidth" || o.sliderLayout == "fullscreen")) punchgs.TweenLite.set(el, { right: 0 - el.outerWidth() + "px", x: o.h_offset + "px" });
		...
		if (el.hasClass("tp-thumbs") || el.hasClass("tp-tabs")) {
			...
			} else if (o.span === true && o.direction === "horizontal") {
	...
	// ADD A BULLET
	var addBullet = function addBullet(container, o, li, opt) {
	...
	// ADD THUMBNAILS
	var addThumb = function addThumb(container, o, li, what, opt) {
		...
		if (container.parent().find(thumbs).length === 0) {
			...
			if (o.position === "outer-top") container.parent().prepend(addcontent);else if (o.position === "outer-bottom") container.after(addcontent);else container.append(addcontent);
	...
	var setONHeights = function setONHeights(o) {
	...
	var biggerNav = function biggerNav(el, a, b, c) {
