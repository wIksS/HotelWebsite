...
(function ($) {
	...
	jQuery.extend(true, _R, {
		...
		updateMarkup: function updateMarkup(layer, o) {
			...
			if (d.start !== undefined && !d.frames_added && d.frames === undefined) {
				...
				if (d.transform_hover) frames.push({ frame: "hover", to: d.transform_hover, style: d.style_hover, speed: oh.speed, ease: oh.anim.ease, splitdelay: d.elementdelay });
				d.frames = frames;
			...
			if (!d.frames_added) {
				...
				if (d.frames !== undefined) {
					for (var i = 0; i < d.frames.length; i++) {
						...
						if (d.frames[i].frame === "hover") d.hoverframeindex = i;else if (d.frames[i].frame === "frame_999" || d.frames[i].frame === "frame_out" || d.frames[i].frame === "last" || d.frames[i].frame === "end") d.outframeindex = i;

						if (d.frames[i].split !== undefined && d.frames[i].split.match(/chars|words|lines/g)) d.splittext = true;
					}
				}
		...
		animcompleted: function animcompleted(_nc, opt) {
			...
			if (t != undefined && t != "none") if (ap == true || ap == "true" || ap == "on" || ap == "1sttime" || an) {
				if (opt.sliderType !== "carousel" || opt.sliderType === "carousel" && opt.carousel.showLayersAllTime === "on" && _nc.closest('li').hasClass("active-revslide") || opt.sliderType === "carousel" && opt.carousel.showLayersAllTime !== "on" && _nc.closest('li').hasClass("active-revslide")) _R.playVideo(_nc, opt);
				...
				if (ap == "no1sttime") _.datasetautoplay = 'on';
  ...
  *********************************************************/
		handleStaticLayers: function handleStaticLayers(_nc, opt) {
			...
			if (s < 0) s = 0;
			if (e < 0) e = opt.realslideamount;
			if (s === 0 && e === opt.realslideamount - 1) e = opt.realslideamount + 1;
  ...
  *************************************/
		animateTheCaptions: function animateTheCaptions(obj) {
			...
			// PREPARE THE LAYERS
			if (index !== undefined && opt.layers[index]) jQuery.each(opt.layers[index], function (i, a) {
				var _t = jQuery(this);
			...
			if (opt.layers["static"]) jQuery.each(opt.layers["static"], function (i, a) {
				var _t = jQuery(this),
				    _ = _t.data();
			...
			if (!preset) if (startSlideAnimAt !== undefined) {
				if (index !== undefined) jQuery.each(opt.timelines[index].layers, function (key, o) {
					var _ = o.layer.data();
					if (o.wrapper === "none" || o.wrapper === undefined) {
						if (o.triggerstate == "keep" && _.triggerstate === "on") _R.playAnimationFrame({ caption: o.layer, opt: opt, frame: "frame_0", triggerdirection: "in", triggerframein: "frame_0", triggerframeout: "frame_999" });else o.timeline.restart();
					}
				});
				if (opt.timelines.staticlayers) jQuery.each(opt.timelines.staticlayers.layers, function (key, o) {
     ...
     console.log(isvisible)*/
					...
					if (ct !== undefined && isvisible) {
						if (triggerstate == "keep") {
							...
							if (_.hoveredstatus !== true) o.timeline.time(ct);
						}
					}
  ...
  ********************************************/
		prepareSingleCaption: function prepareSingleCaption(obj) {
			...
			_._lig = _._lig === undefined ? _nc.hasClass("rev_layer_in_group") ? _nc.closest('.rev_group') : _nc.hasClass("rev_layer_in_column") ? _nc.closest('.rev_column_inner') : _nc.hasClass("rev_column_inner") ? _nc.closest(".rev_row") : "none" : _._lig, _._column = _._column === undefined ? _nc.hasClass("rev_column_inner") ? _nc.closest(".rev_column") : "none" : _._column, _._row = _._row === undefined ? _nc.hasClass("rev_column_inner") ? _nc.closest(".rev_row") : "none" : _._row, _._ingroup = _._ingroup === undefined ? !_nc.hasClass('rev_group') && _nc.closest('.rev_group') ? true : false : _._ingroup;
			...
			if (_._li === undefined) if (_nc.hasClass("tp-static-layer")) {
				_._isstatic = true;
			...
			if (_._togglelisteners === undefined && _nc.find('.rs-toggled-content')) {
				...
				if (_.actions === undefined) _nc.click(function () {
					_R.swaptoggleState(_nc);
				});
			} else {
				_._togglelisteners = false;
			}

			if (opt.sliderLayout == "fullscreen") obj.offsety = _._gh / 2 - opt.gridheight[opt.curWinRange] * opt.bh / 2;

			if (opt.autoHeight == "on" || opt.minHeight != undefined && opt.minHeight > 0) obj.offsety = opt.conh / 2 - opt.gridheight[opt.curWinRange] * opt.bh / 2;;
			...
			if (opt.debugMode) {
				...
				_nc.on("hover, mouseenter", function () {
					var ltxt = "",
					    spa = 0;
					if (_nc.data()) jQuery.each(_nc.data(), function (key, val) {
			...
			if (layervisible === "off" || _._gw < opt.hideCaptionAtLimit && _.captionhidden == "on" || _._gw < opt.hideAllCaptionAtLimit) _._pw.addClass("tp-hidden-caption");else _._pw.removeClass("tp-hidden-caption");
			...
			if (_.thumbimage != undefined && _.videoposter == undefined) _.videoposter = _.thumbimage;
			...
			if (_nc.find('img').length > 0) {
				...
				ww = makeArray(im.data('ww'), opt)[opt.curWinRange] || makeArray(im.data('ww'), opt) || "auto", hh = makeArray(im.data('hh'), opt)[opt.curWinRange] || makeArray(im.data('hh'), opt) || "auto";
				...
				if (_._responsive !== "off") {
					if (_._ba != "grid" && wful) {
						if (jQuery.isNumeric(ww)) im.css({ width: ww + "px" });else im.css({ width: ww });
					} else if (jQuery.isNumeric(ww)) im.css({ width: ww * opt.bw + "px" });else im.css({ width: ww });

					if (_._ba != "grid" && hful) {
						if (jQuery.isNumeric(hh)) im.css({ height: hh + "px" });else im.css({ height: hh });
					} else if (jQuery.isNumeric(hh)) im.css({ height: hh * opt.bh + "px" });else im.css({ height: hh });
				} else {
					im.css({ width: ww, height: hh });
				}

				if (_._ingroup && _._nctype !== "row") {
					if (ww !== undefined && !jQuery.isNumeric(ww) && jQuery.type(ww) === "string" && ww.indexOf("%") > 0) punchgs.TweenLite.set([_._lw, _._pw, _._mw], { minWidth: ww });

					if (hh !== undefined && !jQuery.isNumeric(hh) && jQuery.type(hh) === "string" && hh.indexOf("%") > 0) punchgs.TweenLite.set([_._lw, _._pw, _._mw], { minHeight: hh });
				}
			}
			...
			if (_nc.hasClass("tp-videolayer") || _nc.hasClass("tp-audiolayer") || _nc.find('iframe').length > 0 || _nc.find(tag).length > 0) {
				...
				if (asprat != undefined && asprat.split(":").length > 1) _R.prepareCoveredVideo(opt, _nc);
    	...
    	hh = (parseFloat(hh)*opt.bh)+"px";
				...
				if (_._ingroup) {
					if (_.videowidth !== null && _.videowidth !== undefined && !jQuery.isNumeric(_.videowidth) && _.videowidth.indexOf("%") > 0) punchgs.TweenLite.set([_._lw, _._pw, _._mw], { minWidth: _.videowidth });
				}
			...
			if (_nc.hasClass("tp-resizeme")) _nc.find('*').each(function () {
			...
			if (_._sfx === "block") if (_._bmask === undefined) {
			...
			if (rtl && !_._slidelink) elx = elx + _.eow;
			...
			if (_._nctype !== "row" && _._nctype !== "column") punchgs.TweenLite.set(_._pw, { zIndex: tpcapindex, top: _.calcy, left: _.calcx, overwrite: "auto" });else if (_._nctype !== "row") punchgs.TweenLite.set(_._pw, { zIndex: tpcapindex, width: _.columnwidth, top: 0, left: 0, overwrite: "auto" });else if (_._nctype === "row") {
			...
			if (_.blendmode !== undefined) punchgs.TweenLite.set(_._pw, { mixBlendMode: _.blendmode });
   	...
   	punchgs.TweenLite.set(_.svgcontainer,{ ... });
			...
			if (_._ingroup && _._nctype !== "row") {
				if (_._groupw !== undefined && !jQuery.isNumeric(_._groupw) && _._groupw.indexOf("%") > 0) punchgs.TweenLite.set([_._lw, _._pw, _._mw], { minWidth: _._groupw });

				if (_._grouph !== undefined && !jQuery.isNumeric(_._grouph) && _._grouph.indexOf("%") > 0) punchgs.TweenLite.set([_._lw, _._pw, _._mw], { minHeight: _._grouph });
			}
		},
  ...
  ********************************************/
		createTimelineStructure: function createTimelineStructure(opt) {
  ...
  ***************************************/
		buildFullTimeLine: function buildFullTimeLine(obj) {
				...
				if (!_nc.hasClass("rs-hover-ready")) {
					...
					_nc.hover(function (e) {
						...
						if (animended) {
							...
							if ($svg.svg) _.hovertimelines.svghoveranim = punchgs.TweenLite.to([$svg.svg, $svg.svg.find('path')], frame.speed / 1000, _.hoversvg.anim);
			...
			if (!obj.regenerate) {
				if (_.current_is_nc_timeline) _.current_timeline = _nc_timeline;
				if (_.current_timeline_pause) _nc_timeline.pause(_.current_timeline_time);else _nc_timeline.time(_.current_timeline_time);
			}
		...
		/////////////////////////////////////
		createFrameOnTimeline: function createFrameOnTimeline(obj) {
			var _nc = obj.caption,
			    _ = _nc.data(),
			    label = obj.label,
			...
			if (timeline.vars.id === undefined) timeline.vars.id = Math.round(Math.random() * 100000);
			if (_._nctype === "column") {
			...
			var $to = frame_index !== _.outframeindex ? getAnimDatas(newAnimObject(), _.frames[frame_index].to, undefined, $splitnow, animobject.length - 1) : _.frames[frame_index].to !== undefined && _.frames[frame_index].to.match(/auto:auto/g) === null ? getAnimDatas(newEndAnimObject(), _.frames[frame_index].to, opt.sdir == 1, $splitnow, animobject.length - 1) : getAnimDatas(newEndAnimObject(), _.frames[_.inframeindex].from, opt.sdir == 0, $splitnow, animobject.length - 1),
			    $from = _.frames[frame_index].from !== undefined ? getAnimDatas($to, _.frames[_.inframeindex].from, opt.sdir == 1, $splitnow, animobject.length - 1) : undefined,
			    // ANIMATE FROM THE VERY FIRST SETTING, OR FROM PREVIOUS SETTING 				
			$elemdelay = _.frames[frame_index].splitdelay,
			    $mask_from,
			    $mask_to;

			if (frame_index === 0 && !obj.fromcurrentstate) $mask_from = getMaskDatas(_.frames[frame_index].mask);else $mask_to = getMaskDatas(_.frames[frame_index].mask);
			...
			} else if (frame_index === 0 && obj.fromcurrentstate) {
				$to.speed = $from.speed;
			}
			...
			if (frame_index === 0 && !obj.fromcurrentstate && _._bmask !== undefined && $sfx !== undefined && $sfx.indexOf("block") >= 0 || frame_index === _.outframeindex && !obj.fromcurrentstate && _._bmask !== undefined && $sfx !== undefined && $sfx.indexOf("block") >= 0) {
			...
			if ($splitnow) var ri = getSplitTextDirs(animobject.length - 1, $splitdir);

			if (frame_index === 0 && !obj.fromcurrentstate) {
				if (_._sfx_in === "block") timeline.add(tweens.content.staggerFromTo(animobject, 0.05, { x: 0, y: 0, autoAlpha: 0 }, { x: 0, y: 0, autoAlpha: 1, delay: $sfx_blockdelay }), label);else {
				...
				if (_._sfx_out === "block" && frame_index === _.outframeindex) {
					...
					for (var si in animobject) {
						...
						if (_.frames[frame_index].color !== undefined) $tanim.color = _.frames[frame_index].color;

						if (_.frames[frame_index].bgcolor !== undefined) $tanim.backgroundColor = _.frames[frame_index].bgcolor;

						timeline.add(tweens.content.to(animobject[ri[si]], $to.speed / 1000, $tanim, $elemdelay * si), label);
					}
				} else {
					if (_.frames[frame_index].color !== undefined) $to.anim.color = _.frames[frame_index].color;

					if (_.frames[frame_index].bgcolor !== undefined) $to.anim.backgroundColor = _.frames[frame_index].bgcolor;
			...
			} else if (frame_index === 0) timeline.add(tweens.mask.set(_._mw, { overflow: "visible" }), label);

			if ($mask_from !== undefined && $mask_to !== undefined && $mask_from !== false && $mask_to !== false) timeline.add(tweens.mask.fromTo(_._mw, $from.speed / 1000, $mask_from.anim, $mask_to.anim, $elemdelay), label);else if ($mask_to !== undefined && $mask_to !== false) timeline.add(tweens.mask.to(_._mw, $to.speed / 1000, $mask_to.anim, $elemdelay), label);

			timeline.addLabel(label + "_end");

			// Reset Hover Effect when Last Frame (Out Animation) ordered
			if (_._gsTransformTo && frame_index === verylastframe && _.hoveredstatus) _.hovertimelines.item = punchgs.TweenLite.to(_nc, 0, _._gsTransformTo);
		...
		////////////////////////////
		endMoveCaption: function endMoveCaption(obj) {
			...
			if (obj.frame !== undefined) nc.timeline.play(obj.frame);else if (!nc.static || obj.currentslide >= nc.removeonslide || obj.currentslide < nc.showonslide) {
				nc.outnow = new punchgs.TimelineLite();
				nc.timeline.pause();
				if (_.visibleelement === true) _R.createFrameOnTimeline({ caption: obj.caption, timeline: nc.outnow, label: "outnow", frameindex: obj.caption.data("outframeindex"), opt: obj.opt, fromcurrentstate: true }).play();
			}

			if (obj.checkchildrens) if (nc.timeline_obj && nc.timeline_obj.dchildren && nc.timeline_obj.dchildren !== "none" && nc.timeline_obj.dchildren.length > 0) for (var q = 0; q < nc.timeline_obj.dchildren.length; q++) {
		...
		/////////////////////////////////
		playAnimationFrame: function playAnimationFrame(obj) {
		...
		removeTheCaptions: function removeTheCaptions(actli, opt) {
			...
			if (opt.layers[index]) jQuery.each(opt.layers[index], function (i, a) {
				allcaptions.push(a);
			});
   	...
   	jQuery.each(opt.layers["static"], function(i,a) { allcaptions.push(a); });*/
			...
			if (allcaptions) jQuery.each(allcaptions, function (i) {
				var _nc = jQuery(this);
 ...
 **********************************************************************************************/
	...
	var tweenOnStart = function tweenOnStart(frame_index, ncobj, pw, _, tl, toanim, _nc, ust, opt) {
		...
		if (ncobj.dchildren !== "none" && ncobj.dchildren !== undefined && ncobj.dchildren.length > 0) {
			if (frame_index === 0) for (var q = 0; q < ncobj.dchildren.length; q++) {
				jQuery(ncobj.dchildren[q]).data('timeline').play(0);
			} else if (frame_index === _.outframeindex) for (var q = 0; q < ncobj.dchildren.length; q++) {
	...
	var tweenOnUpdate = function tweenOnUpdate(label, id, pw, _, tl, frame_index, toanim, ust, _nc, opt) {
		...
		if (_.hoveranim !== undefined && _._gsTransformTo === false) {
			...
			if (_.cssobj.styleProps.css === undefined) _._gsTransformTo = jQuery.extend(true, {}, _.cssobj.styleProps, _._gsTransformTo);else _._gsTransformTo = jQuery.extend(true, {}, _.cssobj.styleProps.css, _._gsTransformTo);
		}

		_.visibleelement = true;
	};

	var tweenOnComplete = function tweenOnComplete(frame_index, frame_max, verylastframe, pw, _, tl, ust, _nc, opt) {
		var data = {};
		data.layer = _nc;
		...
		if (data.eventtype === "leftstage") if (_R.stopVideo) _R.stopVideo(_nc, opt);
	...
	/////////////////////////////////////////////
	var getTLInfos = function getTLInfos(obj) {
		var _ = {};
	...
	var getSplitTextDirs = function getSplitTextDirs(alen, d) {
		var ri = new Array();

		switch (d) {
			case "forward":
			case "random":
				for (var si = 0; si <= alen; si++) {
					ri.push(si);
				}
	...
	/////////////////////////////////////////////
	var getCycles = function getCycles(anim) {
		var _ = {};
		for (var a in anim) {
	...
	/////////////////////////////////////
	var newAnimObject = function newAnimObject(a) {
	...
	var newSVGHoverAnimObject = function newSVGHoverAnimObject() {
		var a = new Object();
		a.anim = new Object();
	...
	var setSVGAnimObject = function setSVGAnimObject(data, a) {
		var customarray = data.split(';');
		if (customarray) jQuery.each(customarray, function (index, pa) {
			var p = pa.split(":");
			var w = p[0],
			    v = p[1];
	...
	var newEndAnimObject = function newEndAnimObject() {
		var a = new Object();
		a.anim = new Object();
		a.anim.x = 0;
		a.anim.y = 0;
	...
	var newHoverAnimObject = function newHoverAnimObject() {
		var a = new Object();
		a.anim = new Object();
		a.speed = 0.2;
		return a;
	...
	var animDataTranslator = function animDataTranslator(val, defval, $split, $splitamount, ext) {
		ext = ext === undefined ? "" : ext;
		if (jQuery.isNumeric(parseFloat(val))) {
			return parseFloat(val) + ext;
		} else if (val === undefined || val === "inherit") {
			return defval + "ext";
	...
	var getBorderDirections = function getBorderDirections(x, o, w, h, top, left, direction) {

		if (!jQuery.isNumeric(x) && x.match(/%]/g)) {
			x = x.split("[")[1].split("]")[0];
			if (direction == "horizontal") x = (w + 2) * parseInt(x, 0) / 100;else if (direction == "vertical") x = (h + 2) * parseInt(x, 0) / 100;
		} else {
	...
	///////////////////////////////////////////////////
	var getAnimDatas = function getAnimDatas(frm, data, reversed, $split, $splitamount) {
		...
		if (customarray) jQuery.each(customarray, function (index, pa) {
			...
			if (reversed && reversed !== "none" && v != undefined && v.length > 0 && v.match(/\(R\)/)) {
				...
				if (v[0] === "[" && v[1] === "-") v = v.replace("[-", "[");else if (v[0] === "[" && v[1] !== "-") v = v.replace("[", "[-");else if (v[0] === "-") v = v.replace("-", "");else if (v[0].match(/[1-9]/)) v = "-" + v;
			}
	...
	/////////////////////////////////
	var getMaskDatas = function getMaskDatas(d) {
		if (d === undefined) return false;

		var o = new Object();
		o.anim = new Object();
		var s = d.split(';');
		if (s) jQuery.each(s, function (index, param) {
			param = param.split(":");
			var w = param[0],
			    v = param[1];
			if (w == "x") o.anim.x = v;
	...
	var makeArray = function makeArray(obj, opt, show) {
		...
		if (!jQuery.isArray(obj) && jQuery.type(obj) === "string" && (obj.split(",").length > 1 || obj.split("[").length > 1)) {
			...
			obj = new Array();
			if (newobj) jQuery.each(newobj, function (index, element) {
	...
	var convertHoverStyle = function convertHoverStyle(t, s) {
		...
		var sp = s.split(";");
		if (sp) jQuery.each(sp, function (key, cont) {
			var attr = cont.split(":");
			if (attr[0].length > 0) {
	...
	////////////////////////////////////////////////
	var getcssParams = function getcssParams(nc, level) {
		...
		if (level == "rekursive") {
			...
			if (pc && nc.css("fontSize") === pc.css("fontSize") && nc.css("fontWeight") === pc.css("fontWeight") && nc.css("lineHeight") === pc.css("lineHeight")) gp = true;
		}
		...
		if (jQuery.inArray(nc.data('layertype'), ["video", "image", "audio"]) === -1 && !nc.is("img")) obj.lineHeight = gp ? pc.data('lineheight') === undefined ? parseInt(pc.css('lineHeight'), 0) || 0 : pc.data('lineheight') : nc.data('lineheight') === undefined ? parseInt(nc.css('lineHeight'), 0) || 0 : nc.data('lineheight');else obj.lineHeight = 0;
		...
		if (obj.styleProps.background === "" || obj.styleProps.background === undefined || obj.styleProps.background === obj.styleProps["background-color"]) delete obj.styleProps.background;
		if (obj.styleProps.color == "") obj.styleProps.color = nc.css("color");

		return obj;
	};

	// READ SINGLE OR ARRAY VALUES OF OBJ CSS ELEMENTS
	var setResponsiveCSSValues = function setResponsiveCSSValues(obj, opt) {
		var newobj = new Object();
		if (obj) jQuery.each(obj, function (key, val) {
	...
	var minmaxconvert = function minmaxconvert(a, m, r, fr) {
	...
	/////////////////////////////////////////////////////////////////
	var calcCaptionResponsive = function calcCaptionResponsive(nc, opt, level, responsive) {
   ...
   nc[0].nodeName=="strong" || nc[0].tagName=="STRONG"*/
		...
		if (!nc.hasClass("tp-splitted")) {
			...
			if (level != "rekursive") {
				...
				if (obj.color != "nopredefinedcolor") punchgs.TweenLite.set(nc, { color: obj.color, overwrite: "auto" });
	...
	var setColumnBgDimension = function setColumnBgDimension(nc, opt) {
	...
	//	CAPTION LOOPS	//
	//////////////////////
	var callCaptionLoops = function callCaptionLoops(el, factor) {
		var _ = el.data();
		...
		if (el.hasClass("rs-pendulum")) {
			if (_._loop_timeline == undefined) {
				...
				_._loop_timeline.append(new punchgs.TweenLite.fromTo(el, speed, { force3D: "auto", rotation: enddeg, transformOrigin: origin }, { rotation: startdeg, ease: easing, onComplete: function onComplete() {
		...
		if (el.hasClass("rs-rotate")) {
			if (_._loop_timeline == undefined) {
				...
				_._loop_timeline.append(new punchgs.TweenLite.fromTo(el, speed, { force3D: "auto", rotation: startdeg, transformOrigin: origin }, { rotation: enddeg, ease: easing, onComplete: function onComplete() {
						_._loop_timeline.restart();
					} }));
			}
		}
		...
		if (el.hasClass("rs-slideloop")) {
			if (_._loop_timeline == undefined) {
				...
				_._loop_timeline.append(new punchgs.TweenLite.fromTo(el, speed, { force3D: "auto", x: xe, y: ye }, { x: xs, y: ys, onComplete: function onComplete() {
						_._loop_timeline.restart();
					} }));
			}
		}
		...
		if (el.hasClass("rs-pulse")) {
			if (_._loop_timeline == undefined) {
				...
				_._loop_timeline.append(new punchgs.TweenLite.fromTo(el, speed, { force3D: "auto", scale: zoomend }, { scale: zoomstart, onComplete: function onComplete() {
						_._loop_timeline.restart();
					} }));
			}
		}
	...
	var killCaptionLoops = function killCaptionLoops(nextcaption) {
		// SOME LOOPING ANIMATION ON INTERNAL ELEMENTS
