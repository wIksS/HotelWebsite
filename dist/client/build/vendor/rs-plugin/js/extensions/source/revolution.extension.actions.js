...
(function ($) {
	...
	jQuery.extend(true, _R, {
		checkActions: function checkActions(_nc, opt, as) {
	...
	//////////////////////////////////////////
	var checkActions_intern = function checkActions_intern(_nc, opt, as) {

		if (as) jQuery.each(as, function (i, a) {
			...
			if (!opt.fullscreen_esclistener) {
				if (a.action == "exitfullscreen" || a.action == "togglefullscreen") {
					jQuery(document).keyup(function (e) {
						if (e.keyCode == 27 && jQuery('#rs-go-fullscreen').length > 0) _nc.trigger(a.event);
					});
			...
			switch (a.action) {
				case "togglevideo":
					jQuery.each(tnc, function (i, _tnc) {
						...
						if (videotoggledby == undefined) videotoggledby = new Array();
				...
				case "togglelayer":
					jQuery.each(tnc, function (i, _tnc) {
						...
						if (layertoggledby == undefined) layertoggledby = new Array();
				...
				case "toggle_mute_video":
					jQuery.each(tnc, function (i, _tnc) {
						...
						if (videomutetoggledby == undefined) videomutetoggledby = new Array();
	...
	function getScrollRoot() {
		var html = document.documentElement,
		    body = document.body,
		    cacheTop = (typeof window.pageYOffset !== "undefined" ? window.pageYOffset : null) || body.scrollTop || html.scrollTop,
		    // cache the window's current scroll position
		root;
	...
	var actionSwitches = function actionSwitches(tnc, opt, a, _nc) {
		switch (a.action) {
			case "scrollbelow":
				...
				punchgs.TweenLite.to(sobj, a.speed / 1000, { _y: opt.c.offset().top + jQuery(opt.li[0]).height() - off, ease: a.ease, onUpdate: function onUpdate() {
			...
			case "stopvideo":
				...
				break;
			case "togglevideo":
				if (tnc.length > 0) if (!_R.isVideoPlaying(tnc, opt)) _R.playVideo(tnc, opt);else if (_R.stopVideo) _R.stopVideo(tnc, opt);
				break;
			case "mutevideo":
			...
			case "toggle_mute_video":
				...
				if (tnc.length > 0) if (_R.isVideoMuted(tnc, opt)) {
			...
			case "simulateclick":
				...
				break;
			case "toggleclass":
				if (tnc.length > 0) if (!tnc.hasClass(a.classname)) tnc.addClass(a.classname);else tnc.removeClass(a.classname);
				break;
			case "gofullscreen":
			case "exitfullscreen":
			case "togglefullscreen":
	...
	var getOffContH = function getOffContH(c) {
		if (c == undefined) return 0;
		if (c.split(',').length > 1) {
			var oc = c.split(","),
			    a = 0;
			if (oc) jQuery.each(oc, function (index, sc) {
				if (jQuery(sc).length > 0) a = a + jQuery(sc).outerHeight(true);
			});
			return a;
		} else {
