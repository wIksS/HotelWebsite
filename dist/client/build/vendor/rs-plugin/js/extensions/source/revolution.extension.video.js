...
(function ($) {
	...
	jQuery.extend(true, _R, {
		...
		preLoadAudio: function preLoadAudio(li, opt) {
			...
			li.find('.tp-audiolayer').each(function () {
				...
				if (element.find('audio').length === 0) {
					obj.src = element.data('videomp4') != undefined ? element.data('videomp4') : '', obj.pre = element.data('videopreload') || '';
		...
		preLoadAudioDone: function preLoadAudioDone(nc, opt, event) {
			...
			if (opt.audioqueue && opt.audioqueue.length > 0) jQuery.each(opt.audioqueue, function (i, obj) {
		...
		resetVideo: function resetVideo(_nc, opt, preset, nextli) {
			var _ = _nc.data();
			switch (_.videotype) {
				case "youtube":
					...
					if (_nc.find('.tp-videoposter').length == 0 && _.bgvideo !== 1 && preset !== true) punchgs.TweenLite.to(_nc.find('iframe'), 0.3, { autoAlpha: 1, display: "block", ease: punchgs.Power3.easeInOut });
				...
				case "html5":
					...
					if (_.volume == "mute" || _R.lastToggleState(_nc.videomutetoggledby) || opt.globalmute === true) video.muted = true;
					break;
			}
		},

		isVideoMuted: function isVideoMuted(_nc, opt) {
			var muted = false,
			    _ = _nc.data();
			switch (_.videotype) {
				case "youtube":
		...
		muteVideo: function muteVideo(_nc, opt) {
			var _ = _nc.data();
			switch (_.videotype) {
				case "youtube":
					try {
		...
		stopVideo: function stopVideo(_nc, opt) {
				...
				case "youtube":
					...
					if (_nc.find('iframe').length == 0) {
						...
						if (_.player.playVideo != undefined) {
							...
							if (_.youtubepausecalled !== true) _.player.playVideo();
						} else {
				...
				case "vimeo":
					...
					if (_nc.find('iframe').length == 0) {
						...
						if (_nc.hasClass("rs-apiready")) {
							...
							if (!f.getPaused()) {
								_nc.data('videoplaywait', setTimeout(function () {
									if (_.vimeopausecalled !== true) _R.playVideo(_nc, opt);
								}, 50));
							} else {
								setTimeout(function () {
		...
		prepareCoveredVideo: function prepareCoveredVideo(opt, _nc) {
		...
		checkVideoApis: function checkVideoApis(_nc, opt, addedApis) {
			...
			if ((_nc.data('ytid') != undefined || _nc.find('iframe').length > 0 && _nc.find('iframe').attr('src').toLowerCase().indexOf('youtube') > 0) && addedApis.addedyt == 0) {
				...
				jQuery('head').find('*').each(function () {
					if (jQuery(this).attr('src') == "https://www.youtube.com/iframe_api") loadit = false;
				});
			...
			if ((_nc.data('vimeoid') != undefined || _nc.find('iframe').length > 0 && _nc.find('iframe').attr('src').toLowerCase().indexOf('vimeo') > 0) && addedApis.addedvim == 0) {
				...
				jQuery('head').find('*').each(function () {
					if (jQuery(this).attr('src') == "https://player.vimeo.com/api/player.js") loadit = false;
				});
		...
		manageVideoLayer: function manageVideoLayer(_nc, opt, recalled, internrecalled) {
			if (_R.compare_version(extension).check === "stop") return false;
			...
			var _ = _nc.data(),
			    ...
			    videotype = videomp != undefined || videowebm != undefined ? "html5" : vidytid != undefined && String(vidytid).length > 1 ? "youtube" : vimeoid != undefined && String(vimeoid).length > 1 ? "vimeo" : "none",
			    tag = _.audio == "html5" ? "audio" : "video",
			    newvideotype = videotype == "html5" && _nc.find(tag).length == 0 ? "html5" : videotype == "youtube" && _nc.find('iframe').length == 0 ? "youtube" : videotype == "vimeo" && _nc.find('iframe').length == 0 ? "vimeo" : "none";
     ...
     */
					...
					if (tag === 'video') {
						...
						if (_R.is_mobile() || _R.isSafari11()) {
							/* updated to account for Safari 11 */
					...
					if (videocontrols == "controls") apptxt = apptxt + ('<div class="tp-video-controls">' + '<div class="tp-video-button-wrap"><button type="button" class="tp-video-button tp-vid-play-pause">Play</button></div>' + '<div class="tp-video-seek-bar-wrap"><input  type="range" class="tp-seek-bar" value="0"></div>' + '<div class="tp-video-button-wrap"><button  type="button" class="tp-video-button tp-vid-mute">Mute</button></div>' + '<div class="tp-video-vol-bar-wrap"><input  type="range" class="tp-volume-bar" min="0" max="1" step="0.1" value="1"></div>' + hfm + '</div>');
     		...
     		httpprefix = "https";		*/
					...
					if (_.videoinline === true || _.videoinline === "true" || _.videoinline === 1 || _nc.hasClass('rs-background-video-layer')) vida = vida + "&playsinline=1";
					var s = getStartSec(_nc.data('videostartat')),
					...
					if (orig.length > 1) {
						...
						if (self.location.href.match(/www/gi) && !orig[1].match(/www/gi)) vida_new = vida_new + "www.";
			...
			if (_.videoposter != undefined && _.videoposter.length > 2 && !noposteronmobile) {
				if (_nc.find('.tp-videoposter').length == 0) _nc.append('<div class="tp-videoposter noSwipe" style="cursor:pointer; position:absolute;top:0px;left:0px;width:100%;height:100%;z-index:3;background-image:url(' + _.videoposter + '); background-size:cover;background-position:center center;"></div>');
				if (_nc.find('iframe').length == 0) _nc.find('.tp-videoposter').click(function () {
	...
	// 	-	VIMEO ADD EVENT /////
	var addEvent = function addEvent(element, eventName, callback) {
		if (element.addEventListener) element.addEventListener(eventName, callback, { capture: false, passive: true });else element.attachEvent(eventName, callback, { capture: false, passive: true });
	};

	var getVideoDatas = function getVideoDatas(p, t, d) {
		var a = {};
		a.video = p;
		a.videotype = t;
		a.settings = d;
	...
	var callPrepareCoveredVideo = function callPrepareCoveredVideo(opt, _nc) {
	...
	var addVideoListener = function addVideoListener(_nc, opt, startnow) {
							...
							"onStateChange": function onStateChange(event) {
								...
								if (event.data == YT.PlayerState.PLAYING) {
									...
									if (!fsmode && (event.data == 0 || event.data == 2) && (_nc.data('showcoveronpause') == "on" && _nc.find('.tp-videoposter').length > 0 || _nc.data('bgvideo') === 1 && _nc.find('.rs-fullvideo-cover').length > 0)) {
										if (_nc.data('bgvideo') === 1) punchgs.TweenLite.to(_nc.find('.rs-fullvideo-cover'), 0.1, { autoAlpha: 1, force3D: "auto", ease: punchgs.Power3.easeInOut });else punchgs.TweenLite.to(_nc.find('.tp-videoposter'), 0.1, { autoAlpha: 1, force3D: "auto", ease: punchgs.Power3.easeInOut });
        ...
        */
								...
								if (isVideoMobile || _R.isSafari11()) {
									...
									if (_nc.hasClass('rs-background-video-layer') || isLayerAutoPlay) {
										...
										if (!isSpecial) {
											...
											_nc.data('mobilevideotimr', setTimeout(function () {
												player.playVideo();
											}, 500));
										}
									}
				...
				case "vimeo":
					...
					f.on('play', function (data) {
						...
						addVidtoList(_nc, opt);
						if (pforv) opt.c.trigger('stoptimer');else opt.videoplaying = false;
						if (_nc.data('volume') == "mute" || _R.lastToggleState(_nc.data('videomutetoggledby')) || opt.globalmute === true) f.setVolume(0);else f.setVolume(parseInt(_nc.data('volume'), 0) / 100 || 0.75);
						_R.toggleState(_.videotoggledby);
					});
	...
	var checkfullscreenEnabled = function checkfullscreenEnabled() {
		try {
	...
	var htmlvideoevents = function htmlvideoevents(_nc, opt, startnow) {
		...
		if (_nc.data('bgvideo') == 1 && (_.videoloop === "none" || _.videoloop === false)) pforv = false;
		...
		if (fullScreenButton != undefined) {
			...
			if (fullScreenButton) addEvent(fullScreenButton, "click", function () {
		...
		addEvent(video, "play", function () {
			...
			if (!_R.is_mobile() && !_R.isSafari11()) {
				...
				if (vol == "mute") video.muted = true;else if (vol != undefined) video.volume = vol;
			}
	...
	var addVidtoList = function addVidtoList(_nc, opt) {
	...
	var remVidfromList = function remVidfromList(_nc, opt) {
		if (opt.playingvideos != undefined && jQuery.inArray(_nc, opt.playingvideos) >= 0) opt.playingvideos.splice(jQuery.inArray(_nc, opt.playingvideos), 1);
	};
})(jQuery);