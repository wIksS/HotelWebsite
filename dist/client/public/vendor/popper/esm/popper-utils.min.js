 ...
 */function a(a, b) {
  ...
  if (!d) return document.body;switch (d.nodeName) {case 'HTML':case 'BODY':
    ...
    e = function e() {
  var a = 0 < arguments.length && void 0 !== arguments[0] ? arguments[0] : 'all';return (a = a.toString(), d.hasOwnProperty(a)) ? d[a] : ('11' === a ? d[a] = -1 !== navigator.userAgent.indexOf('Trident') : '10' === a ? d[a] = -1 !== navigator.appVersion.indexOf('MSIE 10') : 'all' === a ? d[a] = -1 !== navigator.userAgent.indexOf('Trident') || -1 !== navigator.userAgent.indexOf('MSIE') : void 0, d.all = d.all || Object.keys(d).some(function (a) {
    return d[a];
  }), d[a]);
};function f(b) {
  if (!b) return document.documentElement;for (var c = e(10) ? document.body : null, d = b.offsetParent; d === c && b.nextElementSibling;) {
  ...
  return p({}, a, { right: a.left + a.width, bottom: a.top + a.height });
}function r(b) {
  var c = {};try {
    if (e(10)) {
      c = b.getBoundingClientRect();var d = k(b, 'top'),
          f = k(b, 'left');c.top += d, c.left += f, c.bottom += d, c.right += f;
    } else c = b.getBoundingClientRect();
  } catch (a) {}var g = { left: c.left, top: c.top, width: c.right - c.left, height: c.bottom - c.top },
      h = 'HTML' === b.nodeName ? o() : {},
      i = h.width || b.clientWidth || g.right - g.left,
      j = h.height || b.clientHeight || g.bottom - g.top,
      l = b.offsetWidth - i,
      n = b.offsetHeight - j;if (l || n) {
    var p = a(b);l -= m(p, 'x'), n -= m(p, 'y'), g.width -= l, g.height -= n;
  }return q(g);
}function s(b, d) {
  var f = Math.max,
      ...
      p = parseFloat(n.borderLeftWidth, 10);g && 'HTML' === d.nodeName && (k.top = f(k.top, 0), k.left = f(k.left, 0));var s = q({ top: j.top - k.top - o, left: j.left - k.left - p, width: j.width, height: j.height });if (s.marginTop = 0, s.marginLeft = 0, !h && i) {
    var t = parseFloat(n.marginTop, 10),
        u = parseFloat(n.marginLeft, 10);s.top -= o - t, s.bottom -= o - t, s.left -= p - u, s.right -= p - u, s.marginTop = t, s.marginLeft = u;
  }return (h && !g ? d.contains(m) : d === m && 'BODY' !== m.nodeName) && (s = l(s, d)), s;
}function t(a) {
  var b = Math.max,
      ...
      h = c ? 0 : k(d),
      i = c ? 0 : k(d, 'left'),
      j = { top: h - e.top + e.marginTop, left: i - e.left + e.marginLeft, width: f, height: g };return q(j);
}function u(c) {
  var d = c.nodeName;return 'BODY' === d || 'HTML' === d ? !1 : !('fixed' !== a(c, 'position')) || u(b(c));
}function v(b) {
  if (!b || !b.parentElement || e()) return document.documentElement;for (var c = b.parentElement; c && 'none' === a(c, 'transform');) {
    c = c.parentElement;
  }return c || document.documentElement;
}function w(a, d, e, f) {
  var g = 4 < arguments.length && void 0 !== arguments[4] && arguments[4],
      h = { top: 0, left: 0 },
      i = g ? v(a) : j(a, d);if ('viewport' === f) h = t(i, g);else {
    var k;'scrollParent' === f ? (k = c(b(d)), 'BODY' === k.nodeName && (k = a.ownerDocument.documentElement)) : 'window' === f ? k = a.ownerDocument.documentElement : k = f;var l = s(k, i, g);if ('HTML' === k.nodeName && !u(i)) {
      var m = o(),
          n = m.height,
  ...
  var f = 5 < arguments.length && arguments[5] !== void 0 ? arguments[5] : 0;if (-1 === a.indexOf('auto')) return a;var g = w(c, d, f, e),
...
}for (var z = 'undefined' != typeof window && 'undefined' != typeof document, A = ['Edge', 'Trident', 'Firefox'], B = 0, C = 0; C < A.length; C += 1) {
  if (z && 0 <= navigator.userAgent.indexOf(A[C])) {
    B = 1;break;
  }
}function i(a) {
...
}var E = z && window.Promise,
    F = E ? i : D;function G(a, b) {
  ...
  } else b = { width: a.offsetWidth, height: a.offsetHeight, left: a.offsetLeft, top: a.offsetTop };return q(b);
}function J(a) {
  ...
  c = c.split('-')[0];var d = J(a),
      e = { width: d.width, height: d.height },
      f = -1 !== ['right', 'left'].indexOf(c),
      g = f ? 'top' : 'left',
      h = f ? 'left' : 'top',
      i = f ? 'height' : 'width',
      j = f ? 'width' : 'height';return e[g] = b[g] + b[i] / 2 - d[i] / 2, e[h] = c === h ? b[h] - d[j] : b[K(h)], e;
}function M(a, b, c) {
  var d = 3 < arguments.length && arguments[3] !== void 0 ? arguments[3] : null,
      e = d ? v(b) : j(b, c);return s(c, e, d);
}function N(a) {
  ...
  var d = void 0 === c ? a : a.slice(0, H(a, 'name', c));return d.forEach(function (a) {
    a['function'] && console.warn('`modifier.function` is deprecated, use `modifier.fn`!');var c = a['function'] || a.fn;a.enabled && O(c) && (b.offsets.popper = q(b.offsets.popper), b.offsets.reference = q(b.offsets.reference), b = c(b, a));
  }), b;
}function V(a, b) {
  ...
  Object.keys(b).forEach(function (c) {
    var d = '';-1 !== ['width', 'height', 'top', 'right', 'bottom', 'left'].indexOf(c) && R(b[c]) && (d = 'px'), a.style[c] = b[c] + d;
  });
}function X(a, b, d, e) {
  var f = 'BODY' === a.nodeName,
      g = f ? a.ownerDocument.defaultView : a;g.addEventListener(b, d, { passive: !0 }), f || X(c(g.parentNode), b, d, e), e.push(g);
}function Y(a, b, d, e) {
  d.updateBound = e, S(a).addEventListener('resize', d.updateBound, { passive: !0 });var f = c(a);return X(f, 'scroll', d.updateBound, d.scrollParents), d.scrollElement = f, d.eventsEnabled = !0, d;
}var Z = { computeAutoPlacement: y, debounce: F, findIndex: H, getBordersSize: m, getBoundaries: w, getBoundingClientRect: r, getClientRect: q, getOffsetParent: f, getOffsetRect: I, getOffsetRectRelativeToArbitraryNode: s, getOuterSizes: J, getParentNode: b, getPopperOffsets: L, getReferenceOffsets: M, getScroll: k, getScrollParent: c, getStyleComputedProperty: a, getSupportedPropertyName: N, getWindowSizes: o, isFixed: u, isFunction: O, isModifierEnabled: P, isModifierRequired: Q, isNumeric: R, removeEventListeners: T, runModifiers: U, setAttributes: V, setStyles: W, setupEventListeners: Y };exports.computeAutoPlacement = y;
exports.debounce = F;
exports.findIndex = H;
exports.getBordersSize = m;
exports.getBoundaries = w;
exports.getBoundingClientRect = r;
exports.getClientRect = q;
exports.getOffsetParent = f;
exports.getOffsetRect = I;
exports.getOffsetRectRelativeToArbitraryNode = s;
