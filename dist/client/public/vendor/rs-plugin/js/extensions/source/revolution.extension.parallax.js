...
(function ($) {
	...
	jQuery.extend(true, _R, {
		...
		checkForParallax: function checkForParallax(container, opt) {
				...
				container.on('mousemove.hoverdir, mouseleave.hoverdir, trigger3dpath', function (event, data) {
					...
					for (var i = 0; i < _.pcontainers.length; i++) {
						var pc = _.pcontainers[i],
						    ...
						    offsh = diffh * pl,
						    offsv = diffv * pl;
						if (_.type == "scroll+mouse" || _.type == "mouse+scroll") punchgs.TweenLite.to(pc, s, { force3D: "auto", x: offsh, ease: punchgs.Power3.easeOut, overwrite: "all" });else punchgs.TweenLite.to(pc, s, { force3D: "auto", x: offsh, y: offsv, ease: punchgs.Power3.easeOut, overwrite: "all" });
					};
					...
					if (_.type == "3D" || _.type == "3d") {
						...
						opt.c.find(sctor).each(function () {
							...
							if (li.hasClass("active-revslide") || opt.sliderType != "carousel") {
								if (_.ddd_bgfreeze != "on" || itslayer) punchgs.TweenLite.to(t, s, { rotationX: offrh, rotationY: -offrv, x: offsh, z: zz, y: offsv, ease: punchgs.Power3.easeOut, overwrite: "all" });else punchgs.TweenLite.to(t, 0.5, { force3D: "auto", rotationY: 0, rotationX: 0, z: 0, ease: punchgs.Power3.easeOut, overwrite: "all" });
							} else punchgs.TweenLite.to(t, 0.5, { force3D: "auto", rotationY: 0, x: 0, y: 0, rotationX: 0, z: 0, ease: punchgs.Power3.easeOut, overwrite: "all" });

							if (event.type == "mouseleave") punchgs.TweenLite.to(jQuery(this), 3.8, { z: 0, ease: punchgs.Power3.easeOut });
						});
					}
				});

				if (_ISM) window.ondeviceorientation = function (event) {
					...
					if (_.type == "3D" || _.type == "3d") {
						...
						opt.c.find(sctor).each(function () {
							...
							if (li.hasClass("active-revslide") || opt.sliderType != "carousel") {
								if (_.ddd_bgfreeze != "on" || itslayer) punchgs.TweenLite.to(t, s, { rotationX: offrh, rotationY: -offrv, x: offsh, z: zz, y: offsv, ease: punchgs.Power3.easeOut, overwrite: "all" });else punchgs.TweenLite.to(t, 0.5, { force3D: "auto", rotationY: 0, rotationX: 0, z: 0, ease: punchgs.Power3.easeOut, overwrite: "all" });
							} else punchgs.TweenLite.to(t, 0.5, { force3D: "auto", rotationY: 0, z: 0, x: 0, y: 0, rotationX: 0, ease: punchgs.Power3.easeOut, overwrite: "all" });
			...
			if (_s.on) {
				if (_s.on_slidebg === "on") for (var i = 0; i < opt.allslotholder.length; i++) {
		...
		scrollHandling: function scrollHandling(opt, fromMouse, speedoverwrite) {
			...
			if (_.type != "3d" && _.type != "3D") {
				if (_.type == "scroll" || _.type == "scroll+mouse" || _.type == "mouse+scroll") if (_.pcontainers) for (var i = 0; i < _.pcontainers.length; i++) {
				...
				if (_.bgcontainers) for (var i = 0; i < _.bgcontainers.length; i++) {
