...
(function ($) {
	...
	jQuery.extend(true, _R, {
		...
		prepareCarousel: function prepareCarousel(opt, a, direction, speed) {
		...
		organiseCarousel: function organiseCarousel(opt, direction, setmaind, unli) {
			...
			if (_.slides) jQuery.each(_.slides, function (i, slide) {
				...
				if (_.fadeout === "on") {
					if (_.vary_fade === "on") tr.autoAlpha = 1 - Math.abs(1 / Math.ceil(_.maxVisibleItems / ha) * d);else switch (_.horizontal_align) {
						case "center":
 ...
 ***************************************************/
	...
	var defineCarouselElements = function defineCarouselElements(opt) {
		var _ = opt.carousel;
		...
		if (_.maxRotation !== 0) if (opt.parallax.type != "3D" && opt.parallax.type != "3d") punchgs.TweenLite.set(_.wrap, { perspective: 1200, transformStyle: "flat" });else punchgs.TweenLite.set(_.wrap, { perspective: 1600, transformStyle: "preserve-3d" });
	...
	var setCarouselDefaults = function setCarouselDefaults(opt) {
		...
		_.wrapwidth = opt.sliderLayout != "auto" ? _.wrapwidth > opt.c.closest('.tp-simpleresponsive').width() ? opt.c.closest('.tp-simpleresponsive').width() : _.wrapwidth : _.wrapwidth > opt.ul.width() ? opt.ul.width() : _.wrapwidth;
		...
		if (_.horizontal_align === "right") punchgs.TweenLite.set(_.wrap, { left: "auto", right: _.wrapoffset + "px", width: _.wrapwidth, overflow: ovf });else punchgs.TweenLite.set(_.wrap, { right: "auto", left: _.wrapoffset + "px", width: _.wrapwidth, overflow: ovf });
	...
	// DIRECTION CHECK
	var dircheck = function dircheck(d, b) {
		return d === null || jQuery.isEmptyObject(d) ? b : d === undefined ? "right" : d;;
	};

	// ANIMATE THE CAROUSEL WITH OFFSETS
	var animateCarousel = function animateCarousel(opt, direction, nsae, speed) {
	...
	var breduc = function breduc(a, m) {
	...
	var getBestDirection = function getBestDirection(a, b, max) {
		var dira = b - a,
		    max,
		    dirb = b - max - a,
		    max;
	...
	var getActiveCarouselOffset = function getActiveCarouselOffset(opt) {
		var ret = 0,
		    _ = opt.carousel;

		if (_.positionanim !== undefined) _.positionanim.kill();

		if (_.slide_globaloffset == "none") _.slide_globaloffset = ret = _.horizontal_align === "center" ? _.wrapwidth / 2 - _.slide_width / 2 : 0;else {
