...
(function (jQuery, undefined) {
	...
	jQuery.fn.extend({
		...
		getRSVersion: function getRSVersion(silent) {
			if (silent === true) {
  ...
  */
	...
	var getNeededScripts = function getNeededScripts(o, c) {
		...
		if (o.addons != undefined && o.addons.length > 0) {
			jQuery.each(o.addons, function (i, obj) {
				if ((typeof obj === "undefined" ? "undefined" : _typeof(obj)) === "object" && obj.fileprefix != undefined) lAjax(obj.fileprefix, o);
			});
		}
	...
	var waitForScripts = function waitForScripts(c, o) {
		...
		if (o.addons != undefined && o.addons.length > 0) {
			jQuery.each(o.addons, function (i, obj) {
				if ((typeof obj === "undefined" ? "undefined" : _typeof(obj)) === "object" && obj.init != undefined) {
 ...
 **************************************/
				...
				opt.c.on('tp-mouseenter', function () {
					if (opt.layersonhover) jQuery.each(opt.layersonhover, function (i, tnc) {
						...
						if (cli.length > 0 && cli.hasClass("active-revslide") || cli.hasClass("processing-revslide") || stl.length > 0) {
							...
							if (_R.playAnimationFrame) _R.playAnimationFrame({ caption: tnc, opt: opt, frame: "frame_0", triggerdirection: "in", triggerframein: "frame_0", triggerframeout: "frame_999" });
	...
	var contWidthManager = function contWidthManager(opt) {
		...
		if (opt.sliderLayout != "auto" && (opt.sliderLayout !== "fullscreen" || opt.fullScreenAutoWidth != "on")) {
			...
			if (opt.sliderLayout == "fullscreen" && opt.fullScreenAutoWidth == "on") punchgs.TweenLite.set(opt.ul, { left: 0, width: opt.c.width() });else punchgs.TweenLite.set(opt.ul, { left: rl, width: opt.c.width() - _R.getHorizontalOffset(opt.c, "both") });
		}

		// put Static Layer Wrapper in Position	
		if (opt.slayers && opt.sliderLayout != "fullwidth" && opt.sliderLayout != "fullscreen") punchgs.TweenLite.set(opt.slayers, { left: rl });
	};
	...
	var containerResized = function containerResized(c, opt) {
		...
		if (!_R.resizeThumbsTabs || _R.resizeThumbsTabs(opt) === true) {
			...
			if (nextsh.data('kenburns') == "on") _R.startKenBurn(nextsh, opt, nextsh.data('kbtl') !== undefined ? nextsh.data('kbtl').progress() : 0);

			if (actsh.data('kenburns') == "on") _R.startKenBurn(actsh, opt, actsh.data('kbtl') !== undefined ? actsh.data('kbtl').progress() : 0);
	...
	var prepareSlides = function prepareSlides(container, opt) {
		...
		opt.allli.find('>img').each(function (j) {
			...
			if (opt.sliderType === "standard" || opt.sliderType === "undefined") newimg.css({ 'opacity': 0 });
		});
					...
					if (element.hasClass("defaultimg")) {
						...
						if (!_R.isIE(8)) {
							if (loadobj.src.indexOf('images/transparent.png') == -1 && loadobj.src.indexOf('assets/transparent.png') == -1 || element.data('bgcolor') === undefined) {
								...
								if (element.data('bgcolor') !== undefined) element.css({ background: element.data('bgcolor') });
							}
						} else {
	...
	var swapSlide = function swapSlide(container) {
		...
		container.trigger('stoptimer');
		opt.cd = 0;
		if (opt.spinner === "off") if (opt.loader !== undefined) opt.loader.css({ display: "none" });else opt.loadertimer = setTimeout(function () {
			if (opt.loader !== undefined) opt.loader.css({ display: "block" });
		}, 50);
	...
	var swapSlideProgress = function swapSlideProgress(defimg, container) {
		...
		if (opt.sliderType == "carousel") {
			...
			mtl = new punchgs.TimelineLite({ onComplete: function onComplete() {
					letItFree(container, nextsh, actsh, nextli, actli, mtl);
				} });
			mtl.add(punchgs.TweenLite.set(nextsh.find('.defaultimg'), { opacity: 0 }));
			mtl.pause();

			if (_R.animateTheCaptions) _R.animateTheCaptions({ slide: nextli, opt: opt, preset: true });
			...
			if (opt.sliderType === "carousel" && opt.carousel.showLayersAllTime === "on") {
				...
				jQuery.each(opt.li, function (i) {
					if (!opt.carousel.allLayersStarted) {
						if (opt.li[i] === nextli) _R.animateTheCaptions({ slide: jQuery(opt.li[i]), maintimeline: mtl, opt: opt, startslideanimat: 0 });else _R.animateTheCaptions({ slide: jQuery(opt.li[i]), opt: opt, startslideanimat: 0 });
					} else _R.animateTheCaptions({ slide: jQuery(opt.li[i]), opt: opt, recall: true });
				});
	...
	var letItFree = function letItFree(container, nextsh, actsh, nextli, actli, mtl) {
		...
		if (opt.sliderType === "carousel") {
			...
			opt.removePrepare = 0;
			punchgs.TweenLite.to(nextsh.find('.defaultimg'), 0.001, { zIndex: 20, autoAlpha: 1, onComplete: function onComplete() {
					removeSlots(container, opt, nextli, 1);
				} });
			if (nextli.index() != actli.index()) {
				punchgs.TweenLite.to(actli, 0.2, { zIndex: 18, autoAlpha: 0, onComplete: function onComplete() {
		...
		nextli.find('.rs-background-video-layer').each(function (i) {
			...
			punchgs.TweenLite.fromTo(_nc, 1, { autoAlpha: 0 }, { autoAlpha: 1, ease: punchgs.Power3.easeInOut, delay: 0.2, onComplete: function onComplete() {
		...
		if (opt.startWithSlide !== undefined && opt.startWithSlide !== "done" && opt.sliderType === "carousel") {
			...
			for (var lis = 0; lis <= opt.li.length - 1; lis++) {
				...
				if (oindex === opt.startWithSlide) callslideindex = lis;
			}
			if (callslideindex !== 0) _R.callingNewSlide(opt.c, callslideindex);
	...
	var restartOnFocus = function restartOnFocus() {
	...
	var lastStatBlur = function lastStatBlur() {
		jQuery('.rev_redraw_on_blurfocus').each(function () {
			var opt = jQuery(this)[0].opt;
	...
	var tabBlurringCheck = function tabBlurringCheck() {
