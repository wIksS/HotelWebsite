...
;window.RevColor = {
	...
	parse: function parse(val, prop, returnColorType) {

		val = RevColor.process(val, true);
		var ar = [];

		if (!prop) ar[0] = val[0];else ar[0] = prop + ': ' + val[0] + ';';

		if (returnColorType) ar[1] = val[1];
		return ar;
	},

	convert: function convert(color, opacity) {
	...
	process: function process(clr, processColor) {
		...
		if (typeof clr !== 'string') {
			...
			try {
				...
				return ['linear-gradient(0deg, rgba(255, 255, 255, 1) 0%, rgba(0, 0, 0, 1) 100%)', 'gradient', {

					'type': 'linear',
					'angle': '0',
					'colors': [{ 'r': '255', 'g': '255', 'b': '255', 'a': '1', 'position': '0', 'align': 'bottom' }, { 'r': '0', 'g': '0', 'b': '0', 'a': '1', 'position': '100', 'align': 'bottom' }]
				}];
			}
		} else if (clr.search('#') !== -1) {
	...
	transparentRgba: function transparentRgba(val, processed) {
	...
	rgbValues: function rgbValues(values, num) {
	...
	rgbaString: function rgbaString(r, g, b, a) {

		return 'rgba(' + r + ', ' + g + ', ' + b + ', ' + a + ')';
	},

	rgbToHex: function rgbToHex(clr) {

		var values = RevColor.rgbValues(clr, 3);
		return RevColor.getRgbToHex(values[0], values[1], values[2]);
	},

	rgbaToHex: function rgbaToHex(clr) {

		var values = RevColor.rgbValues(clr, 4);
		return [RevColor.getRgbToHex(values[0], values[1], values[2]), values[3]];
	},

	getOpacity: function getOpacity(val) {

		return parseInt(RevColor.rgbValues(val, 4)[3] * 100, 10) + '%';
	},

	getRgbToHex: function getRgbToHex(r, g, b) {

		return '#' + ("0" + parseInt(r).toString(16)).slice(-2) + ("0" + parseInt(g).toString(16)).slice(-2) + ("0" + parseInt(b).toString(16)).slice(-2);
	},

	joinToRgba: function joinToRgba(val) {

		val = val.split('||');
		return RevColor.convert(val[0], val[1]);
	},

	processRgba: function processRgba(hex, opacity) {

		hex = hex.replace('#', '');

		var hasOpacity = typeof opacity !== 'undefined',
		    rgb = hasOpacity ? 'rgba' : 'rgb',
		    color = rgb + '(' + parseInt(hex.substring(0, 2), 16) + ', ' + parseInt(hex.substring(2, 4), 16) + ', ' + parseInt(hex.substring(4, 6), 16);

		if (hasOpacity) color += ', ' + (parseInt(opacity, 10) * 0.01).toFixed(2).replace(/\.?0*$/, '') + ')';else color += ')';

		return color;
	},

	processGradient: function processGradient(obj) {
	...
	sanitizeGradient: function sanitizeGradient(obj) {
...
(function ($) {
	...
	function newPreset(val, core, cls, grad) {
		...
		if (!$.isPlainObject(val)) {
			...
			for (var prop in val) {
				...
				if (!isNaN(prop)) {
					...
					titl = prop.replace(/_/g, ' ').replace(/\b\w/g, function (chr) {
		...
		if (val !== 'blank') {
			...
			var shell = '<span class="rev-cpicker-color tptip' + cls + '" data-title="' + titl + '" data-color="' + val + '">' + '<span class="rev-cpicker-preset-tile"></span>' + '<span class="rev-cpicker-preset-bg" style="background: ' + val + '"></span>';
	...
	function writePresets(container, colors) {
		...
		for (var i = 0; i < rows; i++) {
			...
			while (colors.length < (i + 1) * presetColumns) {
	...
	function onChange(gradient, color, trans) {
		...
		if (!gradient) {
			...
			if (hex === 'transparent') color = 'transparent';else if (opacity === '100%') color = RevColor.sanitizeHex(hex);else color = RevColor.processRgba(hex, opacity);
		}

		var isTrans = color === 'transparent',
		    val = !isTrans ? color : '';

		if (!gradient) colorBtn.data('state', color);else gradBtn.data('state', color);

		if (!isTrans) currentColor[0].style.background = val;else currentColor.css('background', val);
	...
	function setPoints(colors) {
		...
		for (var i = 0; i < len; i++) {
			...
			clr = colors[i];
			align = clr.align;
			frag.appendChild(newPoint(align, clr.position, getRgbVal(clr), getBgVal(clr, align)));
		}
	...
	function newPoint(align, pos, val, bg) {
		...
		if (typeof val === 'string') el.setAttribute('data-color', val);else $(el).data('gradient', val);
		...
		if (align === 'bottom') {
			...
			el.innerHTML = '<span class="rev-cpicker-point-triangle" style="border-bottom-color: ' + bg + '"></span>' + '<span class="rev-cpicker-point-square" style="background: ' + bg + '"></span>';
		} else {

			el.innerHTML = '<span class="rev-cpicker-point-square" style="background: ' + bg + '"></span>' + '<span class="rev-cpicker-point-triangle" style="border-top-color: ' + bg + '"></span>';
		}
	...
	function checkGroup() {
		...
		if (len > presetColumns) $('#' + this.id + '-btn').addClass('multiplerows');else $('#' + this.id + '-btn').removeClass('multiplerows');
	...
	function calculatePoints() {
		...
		for (var i = 0; i < len; i++) {
			...
			if (tpe === 'bottom') adjustAlpha(point, begin, end);else adjustColor(point, begin, end);
		}
	...
	function getPoints(i) {
		...
		if (onReverse) {
			...
			if (pos < 50) pos += (50 - pos) * 2;else pos -= (pos - 50) * 2;
		...
		if (curPoint && curPoint[0] !== this) {
			...
			if (align === 'bottom') botPoints[botPoints.length] = pos;else topPoints[topPoints.length] = pos;
		}
	}
	...
	function onDrag(e, ui) {
		...
		if (dragPoint === 'bottom') colorLoc.val(pos + '%').trigger('keyup');else opacityLoc.val(pos + '%').trigger('keyup');
	}
	...
	function onDragStop() {
		...
		if (dragPoint === 'bottom') colorLoc.trigger('focusout');else opacityLoc.trigger('focusout');
	}
	...
	function init() {
		...
		mainContainer.draggable({ containment: 'window', handle: '.rev-cpicker-draggable', stop: function stop() {
		...
		cPicker.on('click.revcpicker', function (e) {
			...
			if (cPicker.hasClass('open')) {
				...
				var targt = e.target,
				    $this = $(targt),
				    ids = targt.id,
				    isTarget = targt.className.search('rev-cpicker-point') !== -1 || ids === 'rev-cpicker-section-right' || ids.search('hit') !== -1 || $this.closest('#rev-cpicker-section-right, #rev-cpicker-point-wrap').length;
		...
		colorIris.wpColorPicker({ palettes: false, width: 267, border: false, hide: false, change: function change(e, ui) {
		...
		gradIris.wpColorPicker({ palettes: false, height: 250, border: false, hide: false, change: function change(e, ui) {
		...
		$('.rev-cpicker-point-location').on('keyup.revcpicker focusout.revcpicker', function (e) {
			...
			while (locations.indexOf(point) !== -1) {
				point += dir;
			}if (evt === 'focusout') {
		...
		$('.rev-cpicker-save-preset-btn').on('click.revcpicker', function () {
			...
			doc.off('revcpicker_onajax_save.revcpicker').on('revcpicker_onajax_save.revcpicker', function (evt, err) {
				...
				if (box.length) pre.insertBefore(box.eq(0));else pre.insertBefore(group.find('.ps-scrollbar-x-rail'));
		...
		directions.on('click.revcpicker', function () {
			...
			if (dir !== 'radial') angle.removeAttr('disabled').val(getDegree(dir));else angle.attr('disabled', 'disabled');
		...
		$('.rev-cpicker-point-delete').on('click.revcpicker', function () {
			...
			if (len <= maxPoints) {
				...
				if (align === 'bottom') hitBottom.removeClass('full');else hitTop.removeClass('full');
			}
		});
		...
		reverse.on('click.revcpicker', function () {
			...
			if (rev) reverse.addClass('checked');else reverse.removeClass('checked');
	...
	var ColorPicker = {
		...
		refresh: function refresh() {
			...
			if ($this.hasClass('rev-cpicker-component')) {
				...
				if (val !== 'transparent') $this.data('tpcp')[0].style.background = colorValue;else $this.data('tpcp').css('background', '');
	...
	$(function () {
		...
		$('body').on('click.revcpicker', '.rev-colorpicker', function () {
			...
			if (value[1] !== 'gradient') colorBtn.data('state', openingValue);else gradBtn.data('state', openingValue);
